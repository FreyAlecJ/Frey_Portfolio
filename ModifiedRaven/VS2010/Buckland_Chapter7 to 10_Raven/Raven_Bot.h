...
private:
  ...
  //amount of damage the bot has inflicted during its lifetime
	int								 m_pain;

  //amount of times the bots get stuck
	int								stuck;
  ...
  //holds id number for tgid
  int								 tgID;

  //total damage bot taken
  int								damageTaken;

  //holds time/date information for use in file creation
  char*								dt;
  std::string						filename;
  ...
  //hash map for ANGER values
  std::map<int, int> anger;

  //hash map for NEMESIS storage
  std::map<int, int> nemisis;

  //hash map for VICTIM storage
  std::map<int, int> victim;

  //hash map holding the recent damage of each bot. This is reset on a bot by bot basis
  //during the update vision function of the sensory memory
  std::map<int, int> recentDamage;
...
public:
  ...
  Raven_Bot(Raven_Game* world, Vector2D pos, int tgSys);
  virtual ~Raven_Bot();
  ...
  int			GetNem(int target);
  int			GetVictim(int target);
  int			GetAnger(int target);
  int			TgID()const { return tgID; }
  int			Pain()const {return m_pain;}
  ...
  void			IncreasePain(unsigned int val);
  void			AddStuck() { stuck += 1; }
  int			GetStuck() { return stuck; }
  void			IncreaseDamageTaken(unsigned int val) { damageTaken += val; }
  int			DamageTaken() { return damageTaken; }


  //resets the damgae of the passed bot id in recent damage. 
  void			ResetDamage(int id);
  int			RecentDamage(int id);
  int			HighestRecentDamage();
  bool			InCombat();
  ...
  //writes all pertainent data for the bot. IE, deaths, score and hash tables
  void			WriteData();
