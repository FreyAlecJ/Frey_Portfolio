...
{       
  ...
  //also a good behavior to add into this mix)
	...
	if (On(evade))
	{
		Raven_Bot* enemy = m_pRaven_Bot->GetTargetBot();
		force = Evade(enemy);
		if (!AccumulateForce(m_vSteeringForce, force)) return m_vSteeringForce;
	}
...
//------------------------------- Evade ----------------------------------
// Given a target, the bot will evade it
//------------------------------------------------------------------------
Vector2D Raven_Steering::Evade(const Raven_Bot* pursuer)
{
	if (pursuer == NULL)
	{
		return Vector2D(0, 0);
	...
	Vector2D ToPursuer = pursuer->Pos() - m_pRaven_Bot->Pos();

	double LookAheadTime = ToPursuer.Length() /
		(m_pRaven_Bot->MaxSpeed() + pursuer->Speed());

	return Flee(pursuer->Pos() + pursuer->Velocity() * LookAheadTime);
}

//---------------------------- Flee --------------------------------------
//Given a target, the bot will flee from that position
//------------------------------------------------------------------------
