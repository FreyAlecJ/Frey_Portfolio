                          ...
                          m_pGraveMarkers(NULL)
{
  ...
  LoadMap(script->GetString("SecondMap"));
...
//-----------------------------Set Pause Timer -------------------------------
void			Raven_Game::setPauseTimer()
{
	pauseTimer = clock();
	duration += pauseTimer - start;
}
...
{ 
  ...
  //don't update if the user has paused the game
	if (m_bPaused)
	{
		if (collectData)
		{
			//every pause write the data for each bot.
			std::list<Raven_Bot*>::const_iterator it;
			for (it = m_Bots.begin(); it != m_Bots.end(); it++)
			{
				(*it)->WriteData();
			}
			ToggleCollect(); //turns collect data off to prevent continue data writing
			TogglePause();
			trials += 1;
			if (trials == 30)
			{
...
//----------------------------- GetBotWithI -----------------------------------
Raven_Bot* Raven_Game::GetBotWithID(int id)
{
...
{ 
  while (NumBotsToAdd--)
  {
    ...
    Raven_Bot* rb = new Raven_Bot(this, Vector2D(), NumBotsToAdd);
