...
Goal_Think::Goal_Think(Raven_Bot* pBot):Goal_Composite<Raven_Bot>(pBot, goal_think)
{
  ...
  double AggresiveDefenseBias = RandInRange(LowRangeOfBias, HighRangeOfBias);
  ...
  m_Evaluators.push_back(new AggressiveDefense_Evaluator(AggresiveDefenseBias));
  m_Evaluators.push_back(new Defense_Evaluator(DefenseBias));
}
...
void Goal_Think::AddGoal_FleeFromEnemy()
{
	if (notPresent(goal_flee_from_enemy))
	{
		RemoveAllSubgoals();
		AddSubgoal(new Goal_FleeFromEnemy(m_pOwner));
	}
}
