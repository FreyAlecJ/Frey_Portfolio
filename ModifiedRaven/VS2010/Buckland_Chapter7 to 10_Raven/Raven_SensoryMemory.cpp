...
void Raven_SensoryMemory::UpdateVision()
{
  ...
  for (curBot = bots.begin(); curBot!=bots.end(); ++curBot)
  {
    ...
    if (m_pOwner != *curBot)
    {
      ...
      {
		//debug_con << m_pOwner->ID() << " Has taken " << m_pOwner->RecentDamage((*curBot)->ID())
			//  << " damagae from ----> Bot " << (*curBot)->ID() << "";
      ...
      {
		m_pOwner->ResetDamage((*curBot)->ID()); //bot not within view, reset the damage
        info.bShootable = false;
        info.bWithinFOV = false;
      }
