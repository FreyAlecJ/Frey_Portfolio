...
private:
  ...
  bool							   collectData; //turned on when P is pressed
  bool                             m_bPaused;
...
public:
  ...
  //returns the bot with the passed id
  Raven_Bot* GetBotWithID(int id);
  ...
  void		  ToggleCollect() { collectData = !collectData; }

  //returns time
  clock_t				GetDuration() { duration = clock(); return duration - start; }
  void			setStart();
  void			setPauseTimer();
